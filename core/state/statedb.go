...
func (s *StateDB) StateDiff(deleteEmptyObjects bool) (root common.Hash, destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte, storages map[common.Hash]map[common.Hash][]byte, codes map[common.Hash][]byte, err error) {
	...
	// Dirty accounts, codes and storage. Upstream replaced the old stateObjectsDirty set and
	// the s.storages map with the per-account mutations map; a non-delete mutation is an
	// account update. Finalise (called by IntermediateRoot above) marks every account touched
	// this block — including storage-only changes — as a mutation, so iterating mutations does
	// not drop any storage diff.
	for addr, op := range s.mutations {
		if op.isDelete() {
			continue
		}
		...
		if obj.dirtyCode {
			codes[common.BytesToHash(obj.CodeHash())] = common.CopyBytes(obj.code)
		}
		// Storage diff. The old s.storages map (committed slot writes, keyed
		// addrHash->slotHash->rlp) is gone upstream; reconstruct it from the state object.
		// pendingStorage holds the block-final slot values and originStorage the block-start
		// committed values; both survive IntermediateRoot (only reset at commit), whereas
		// uncommittedStorage is cleared inside updateTrie. A slot changed this block iff
		// pendingStorage[key] != originStorage[key]. Encode each value exactly as updateTrie
		// writes it to the storage trie: prefix-zero-trimmed then RLP, with the zero value
		// meaning slot deletion (nil) — matching the addrHash->slotHash->rlp format the
		// indexer consumes.
		for key, value := range obj.pendingStorage {
			if value == obj.originStorage[key] {
				continue // unchanged or reverted within the block
			}
			slotHash := crypto.Keccak256Hash(key[:])
			if storages[addrHash] == nil {
				storages[addrHash] = make(map[common.Hash][]byte)
			}
			if value == (common.Hash{}) {
				storages[addrHash][slotHash] = nil
				continue
			}
			enc, encErr := rlp.EncodeToBytes(common.TrimLeftZeroes(value[:]))
			if encErr != nil {
				return common.Hash{}, nil, nil, nil, nil, encErr
			}
			storages[addrHash][slotHash] = enc
		}
	}
	return
}
